...
2. **47문항이 기본 내장**되어 있어 바로 플레이할 수 있습니다.
...
- 총 **47문항** (상 19 / 중 21 / 신곡 7) — 전곡 가사·영상 입력, 서브유닛·페어링·솔로곡 대폭 보강
...
- **🗂 저장 슬롯 (여러 벌 저장 · 🔒 비밀번호)** — 저장 기능을 이곳으로 **일원화**했습니다. 비밀번호(`19950719`)로
  잠금 해제 후 현재 구성을 **이름 붙여 여러 개 저장**하고 언제든 불러올 수 있습니다(예: "1차 모임용", "상 난이도 세트").
  각 슬롯은 이 기기에 보관되며, **파일** 버튼으로 개별 내려받기도 가능합니다.
- **💾 배포본 저장(커밋용)** — 저장 슬롯 안의 버튼. 현재 설정을 `svt-data.json`으로 내려받습니다.
  이 파일을 저장소에 **커밋**하면 배포된 웹 페이지의 **기본값**에 반영됩니다. (자세한 흐름은 아래 참고)
...
> 🎬 **영상/음원 링크 — 47곡 전곡 등록 완료.** 공식 MV가 있으면 MV를, 없으면 공식 오디오(음원)나 가사 영상을 사용했습니다.
