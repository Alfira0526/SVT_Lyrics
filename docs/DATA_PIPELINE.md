...
- **추천 가사(`--lyrics`)·유튜브 영상(`--mv`)·추천 시작구간(`--time`)은 기본으로 연결**한다(퀴즈 출제·정답 재생용). 셋 중 하나라도 비면 스크립트가 경고를 출력한다. 당장 못 채우면 `?staging=1` 편집기에서 채운다.
  ...
  - **한글 독음(`read`)**: **비한국어 곡은 권장.** 원어 음성팩이 없는 기기에서는 이 독음을 한국어 음성으로 대체 낭독한다(무음/설치강권으로 인한 이탈 방지). 영어→한글 발음, 중국어→표준중국어 한글 근사. `content[id].read` 또는 `add-song.mjs --read "..."`. 원어 음성이 있는 기기는 원어로 읽고, 없으면 자동으로 이 값을 사용.
